...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/26 12:03:14 by gvigano           #+#    #+#             */
/*   Updated: 2025/07/26 12:03:15 by gvigano          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main() {
	...
	std::cout << "\nTEST 28: DiamondTrap Default Constructor:" << std::endl;
	DiamondTrap defaultDiamond;

	std::cout << "\nTEST 29: DiamondTrap Parametric Constructor:" << std::endl;
	DiamondTrap namedDiamond("Ultimate");

	std::cout << "\nTEST 30: DiamondTrap Copy Constructor:" << std::endl;
	DiamondTrap copiedDiamond(namedDiamond);

	std::cout << "\nTEST 31: DiamondTrap Assignment Operator:" << std::endl;
	DiamondTrap assignedDiamond;
	assignedDiamond = namedDiamond;

	std::cout << "\nTEST 32: DiamondTrap Attack Function:" << std::endl;
	namedDiamond.attack("Enemy1");
	namedDiamond.attack("Enemy2");
	namedDiamond.attack("Enemy3");

	std::cout << "\nTEST 33: DiamondTrap Who Am I Function:" << std::endl;
	namedDiamond.whoAmI();
	copiedDiamond.whoAmI();

	std::cout << "\nTEST 34: DiamondTrap Inherited Functions:" << std::endl;
	namedDiamond.takeDamage(30);
	namedDiamond.beRepaired(15);
	namedDiamond.takeDamage(40);

	std::cout << "\nTEST 35: DiamondTrap Special Functions:" << std::endl;
	namedDiamond.ScavTrap::guardGate();
	namedDiamond.FragTrap::highFivesGuys();

	std::cout << "\nTEST 36: DiamondTrap Values Test:" << std::endl;
	DiamondTrap valueDiamondTest("DiamondValueBot");
	for (int i = 1; i <= 51; i++) {
		std::cout << "action number " << i << " : ";
		valueDiamondTest.attack("EnergyTest");
	}

	std::cout << "\nTEST 37: DiamondTrap Death Test:" << std::endl;
	DiamondTrap diamondDeathTest("DiamondDeathBot");
	diamondDeathTest.takeDamage(100);
	diamondDeathTest.attack("Someone");
	diamondDeathTest.whoAmI();
	diamondDeathTest.ScavTrap::guardGate();
	diamondDeathTest.FragTrap::highFivesGuys();
	diamondDeathTest.beRepaired(10);
